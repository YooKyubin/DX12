...
shared_ptr<Mesh> Mesh::CreateFromFBX(const FbxMeshInfo* meshInfo, class FBXLoader& loader)
{
...
#pragma region SkinData
	if (IsAnimMesh())
	{
		...
		for (int32 i = 0; i < animCount; i++)
		{
			...
			frameParams.resize(_bones.size() * animClip.frameCount); // 1차원 vector이지만 2차원처럼 사용하는 모습 그거임 그거 ppt 2차원 배열 
