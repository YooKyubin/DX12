...
ParticleSystem::ParticleSystem() : Component(COMPONENT_TYPE::PARTICLE_SYSTEM)
{
	...
	_particleBuffer->Init(sizeof(ParticleInfo), _maxParticle); // g_particle

	_computeSharedBuffer = make_shared<StructuredBuffer>();
	_computeSharedBuffer->Init(sizeof(ComputeSharedInfo), 1); // g_shared
...
void ParticleSystem::Render()
{
	...
	// compute shader에서 계산한 결과인 _particleBuffer를 다시 t9으로 넘기는 모습
